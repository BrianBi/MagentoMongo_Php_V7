 ...
 * This shell depends on the usage of the Yaoli_Mongogento suite.
 *
 * @category  Yaoli
 * @package   Yaoli_Shell
 * @author    Brian.Bi
...
class Yaoli_Shell_Mongoify extends Mage_Shell_Abstract
{
    /** @var int the default store Id */
    const DEFAULT_STORE_VIEW = 0;

    /** @var string The prefix used by Yaoli_MongoCatalog to store attributes */
    ...
    /** @var null|Yaoli_MongoCatalog_Model_Resource_Override_Catalog_Product $_catalogResource Mongo dedicated resource model */
     ...
     * Simple constructor
     *
     * @return Yaoli_Shell_Mongoify
     */
    public function __construct()
    {
        parent::__construct();

        /** @var Yaoli_MongoCatalog_Model_Resource_Override_Catalog_Product $catalogResource */
        if (Mage::helper('core')->isModuleEnabled("Yaoli_MongoCatalog")) {
            ...
            $this->_catalogResource = new Yaoli_MongoCatalog_Model_Resource_Override_Catalog_Product();
        }
    ...
    protected function _migrateGalleriesValues()
    {
         ...
         *
         * 1、select * from catalog_product_entity_media_gallery where entity_id=1505;
         * 2、select * from catalog_product_entity_media_gallery_value where value_id=1429;
         */
    ...
    public function galleryCallback($args)
    {
        ...
        // value_id is md5ified on the Yaoli_MongoCatalog module
        ...
        $updateFilter = array('_id' => intval($entityId));
        $updateValue = array(
        ...
        // Update the document
        $update = $this->_catalogResourceCollection->updateOne($updateFilter, $updateValue, array('upsert' => true));
        echo ".";
        if ($update) {
    ...
    protected function _cleanCache()
    {
        ...
        if (Mage::helper("core")->isModuleEnabled("Yaoli_MageCache")) {
            Mage::getSingleton('Yaoli_magecache/processor')->cleanCache();
        }
    }
    ...
    protected function _cleanAttributesTables()
    {
        ...
        // @see Yaoli_MongoCatalog_Model_Resource_Override_Catalog_Product::getSqlAttributesCodes()
        $expectedNumber = (
        ...
        if (empty($sqlAttributesIds) || (count($sqlAttributesIds) < $expectedNumber)) {
            Mage::throwException(
                ...
                . " than expected in the native Yaoli_MongoCatalog module ({$expectedNumber}) ..."
...
// Run the script
$shell = new Yaoli_Shell_Mongoify();
$shell->run();