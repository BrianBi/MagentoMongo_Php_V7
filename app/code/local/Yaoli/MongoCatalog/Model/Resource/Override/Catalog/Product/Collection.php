 ...
 */
class Yaoli_MongoCatalog_Model_Resource_Override_Catalog_Product_Collection extends Mage_Catalog_Model_Resource_Product_Collection
{
    /**
     ...
     *
     * @return Yaoli_MongoCore_Model_Resource_Override_Catalog_Product_Collection Self reference
     */
        ...
        if (!empty($this->_itemsById)) {
            ...
            if (is_null($this->_loadedDocuments)) {
                ...
                foreach ($documentIds as $key => $value) {
                    ...
                    $documentIds[$key] = intval($value);
                }

                $idFilter = array('_id' => array('$in' => $documentIds));
                $storeNew = array();
                if(is_array($storeFilter) && count($storeFilter) > 0)
                {
                    foreach ($storeFilter as $store)
                    {
                        $storeNew[$store] = 1;
                    }
                }
                ...
                $cursor = $this->_getDocumentCollection()->find($idFilter, $options);
